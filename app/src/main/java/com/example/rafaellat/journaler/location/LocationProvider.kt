package com.example.rafaellat.journaler.location
...
object LocationProvider {
    ...
    private val locationListener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
            Log.i(
                tag,
                String.format(Locale.ENGLISH, "Location [lat: %s][long: %s]", location.latitude, location.longitude)
            )
    ...
    fun subscribe(subscriber: LocationListener): Boolean {
        val result = doSubcribe(subscriber)
    ...
    //check if permissions are properly fulfilled
    ...
    //responsible for subscribing in location updates to interested third parties

    private fun doSubcribe(listener: LocationListener): Boolean {
        var result = true
        val iterator = listeners.iterator()
        while (iterator.hasNext()) {
            val reference = iterator.next()
