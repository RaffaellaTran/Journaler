...
object Db : Crud<DbModel> {
        ...
        what.forEach { item ->
            when (item) {
                is Entry -> {
                    ...
                    values.put(DbHelper.COLUMN_MESSAGE, item.message)
                    values.put(
                        DbHelper.COLUMN_LOCATION_LATITUDE, item.location?.latitude
                    )
                    values.put(
                        DbHelper.COLUMN_LOCATION_LONGITUDE, item.location?.longitude
                    )
                    when (item) {
                        is Note -> {
                            table = DbHelper.TABLE_NOTES
    ...
    override fun delete(what: DbModel): Boolean = delete(listOf(what))
